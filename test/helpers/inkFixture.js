...
function createTransform(transform = {}) {
  return Buffer.concat([
    floatField(1, transform.a ?? 1), floatField(2, transform.b ?? 0),
    floatField(3, transform.c ?? 0), floatField(4, transform.d ?? 1),
    floatField(5, transform.tx ?? 0), floatField(6, transform.ty ?? 0),
  ]);
}
...
function createInkDefinition(ink) {
  ...
  return Buffer.concat([
    bytesField(1, Buffer.concat([floatField(1, color.r), floatField(2, color.g), floatField(3, color.b), floatField(4, color.a)])),
...
function createPath(stroke) {
  ...
  points.forEach((point, index) => {
    ...
    if (pressureAt >= 0 && pressureAt + 2 <= stride) pointData.writeUInt16LE(Math.round((point.pressure ?? 0.5) * 1000), offset + pressureAt);
  });
  return Buffer.concat([varintField(3, points.length), varintField(4, format), bytesField(7, pointData)]);
}
...
function createRenderFragment(fragment) {
  ...
  return Buffer.concat([bytesField(7, createTransform(fragment.transform)), bytesField(10, pointData)]);
}

function createStroke(stroke) {
  const fields = [varintField(4, stroke.inkIndex ?? 0), bytesField(5, createPath(stroke)), bytesField(7, createTransform(stroke.transform ?? { tx: 10, ty: 20 }))];
...
function createInkArchive(options = {}) {
  ...
  const strokes = options.strokes ?? [{ inkIndex: 0, points: options.points, pointStride: options.pointStride, format: options.format, transform: options.transform, fragments: options.fragments }];
  const payload = [];
