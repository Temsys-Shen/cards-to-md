var __MN_WEB_API_MNCardsToMDAddon = (function () {
  const FRAME_CONFIG_KEY = "mn_web_template_mncardstomdaddon_frame_config";
  const PANEL_ON_KEY = "mn_web_template_mncardstomdaddon_panel_on";
  ...
  function resolveWebEntryURL(mainPath) {
    const devServerURL = __MNGetWebDevServerURL_MNCardsToMDAddon();
    if (devServerURL) {
  ...
  function setupWebPanelUI(controller) {
    controller.navigationItem.title = "Cards To MD";
    controller.view.autoresizingMask = 0;
    ...
    controller.titleLabel.text = "Cards To MD";
  ...
  function dispatchBridgeCommand(controller, message) {
    const commandTable = __MN_WEB_BRIDGE_COMMANDS_MNCardsToMDAddon.commands;
  ...
  const panelControllerClass = JSB.defineClass("MNWebPanelController_MNCardsToMDAddon : UIViewController <UIWebViewDelegate>", {
